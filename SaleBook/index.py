...
from SaleBook import dao, utils
...
def checkout_in_store():
    """
    Create new order online with status waiting to 'pay on pickup in store'.
    ...
    if request.method.__eq__('POST'):
        ...
        if cart:
            order = dao.add_order_pending(customer_id=current_user.id)
            for c in cart:
...
def commit_checkout_offline():
    ...
    if dao.access_check_employee(current_user.id):
        ...
        dao.set_order_success(order_id, current_user.id)
...
def add_new_book():
    ...
    if dao.access_check(current_user.id):
        ...
        import_details = session.get(session_name)
        ...
        for key in import_details:
            if name in import_details:
                continue
            elif barcode == import_details[key]['barcode']:
                return jsonify({'error': 'barcode existed'}), 500
...
def commit_import_book():
    ...
    """
    # tính total price
    if dao.access_check_importer(current_user.id) and session['import_detail']:
        ...
        os.makedirs(upload_folder, exist_ok=True)  # Tạo lại thư mục trống
...
def add_author():
    if dao.access_check_importer(current_user.id):
        ...
        if request.method.__eq__('POST'):
            ...
            return render_template('importer/add_author.html', successfully=s)
...
def commit_invoice_book():
    ...
    if dao.access_check_employee(current_user.id):
        ...
        order = dao.add_order_offline(employee_id=current_user.id)
        for key in session['sale_book']:
            order_detail = session['sale_book'][key]
...
def sale_history():
    if dao.access_check_employee(current_user.id):
        orders = dao.get_all_order_of_employee(current_user.id)
        return render_template('employee/sale_history.html', orders=orders)
    else:
        return render_template('err_auth.html', err='Bạn không có quyền truy cập')
